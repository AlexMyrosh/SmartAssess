...
namespace FluentValidator.Exam
{
    public class ExamViewModelValidator : AbstractValidator<ExamViewModel>
    {
        public ExamViewModelValidator(IStringLocalizer<ExamViewModelValidator> localizer)
        {
            ...
            RuleFor(x => x.ExamDuration).NotNull().WithMessage(localizer["ExamDurationValueRequired"]);
            RuleFor(x => x.FinalGradeCalculationMethod).NotNull().WithMessage(localizer["FinalGradeCalculationMethodRequired"]);
            ...
            RuleFor(x => x.MinimumPassGrade).NotNull().WithMessage(localizer["MinPassGradeShouldBeLessThanTotalQuestionsGrade"]).LessThanOrEqualTo(x => x.Questions.Sum(q => q.MaxGrade)).WithMessage(localizer["MinPassGradeShouldBeLessThanTotalQuestionsGrade"]);
        }
    }
}
